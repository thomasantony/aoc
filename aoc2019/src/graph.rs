...
impl Graph {
    ...
    pub fn djikstra(&self, start_node: NodeIndex, goal_node: NodeIndex) -> Vec<NodeIndex> {
        ...
        while vertices.len() > 0 {
            let (current_node_idx, current_node) = vertices
                .iter()
                .enumerate()
                .min_by_key(|(_, n)| dist.get(n))
                .unwrap();
...
pub trait BFSGraph<NodeType: Debug + PartialEq + Clone + Hash + Eq> {
    fn successors(&self, node: &NodeType) -> Vec<NodeType>;
    fn search_with<F>(&self, start_node: &NodeType, is_goal_fn: F) -> Vec<NodeType>
    where
        F: Fn(&NodeType) -> bool,
    {
        use std::collections::HashMap;
...
pub fn djikstra_generic<
    NodeType: Debug + PartialEq + Clone + Hash + Eq,
    G: GenericGraph<NodeType>,
>(
    map: &G,
    start_node: NodeType,
    goal_node: NodeType,
