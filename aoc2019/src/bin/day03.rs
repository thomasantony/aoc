...
fn h_v_line_intersection(h_line: &Line, v_line: &Line) -> Option<Intersection> {
    ...
    let lines_cross =
        x_min < v_line.p1.x && v_line.p1.x < x_max && y_min < h_line.p1.y && h_line.p1.y < y_max;
    if lines_cross {
        ...
        Some(Intersection {
            point: point,
            path_length: h_line.length_from_start
                + h_intersect_dist
                + v_line.length_from_start
                + v_intersect_dist,
        })
    } else {
        None
...
fn parse_paths(lines: Vec<&str>) -> Vec<Path> {
    let path_def_list = lines
        .iter()
        .map(|line_def_string| line_def_string.split(","));
...
fn find_intersections(paths: Vec<Path>) -> Vec<Intersection> {
    ...
    let all_intersections: Vec<Intersection> = h1v2
        .chain(h2v1)
...
fn get_min_dist(intersections: &Vec<Intersection>) -> Option<i32> {
    intersections
        .iter()
        .map(|pt| pt.point.x.abs() + pt.point.y.abs())
        .min()
}
fn get_min_path_length(intersections: &Vec<Intersection>) -> Option<i32> {
    intersections.iter().map(|pt| pt.path_length).min()
}
fn main() {
    ...
    println!(
        "Part B: {:?}",
        path_length.expect("Failed to find min path length")
    );
}
