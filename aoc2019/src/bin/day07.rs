...
fn main() {
    ...
    let output_b = find_optimal_thruster_signal(
                &data, 
              compute_feedback_thruster_signal, 
            5..10);
...
mod tests {
    ...
    fn unit_tests_day_07_part_a() {
        ...
        assert_eq!(find_optimal_thruster_signal(&program_1, compute_thruster_signal, 0..5), 43210);

        let program_2 = vec![3,23,3,24,1002,24,10,24,1002,23,-1,23,
                            101,5,23,23,1,24,23,23,4,23,99,0,0];
        assert_eq!(compute_thruster_signal(& [0, 1, 2, 3, 4], &program_2), 54321);
        assert_eq!(find_optimal_thruster_signal(&program_2, compute_thruster_signal, 0..5), 54321);
        ...
        assert_eq!(find_optimal_thruster_signal(&program_3, compute_thruster_signal, 0..5), 65210);
    }
    #[test]
    fn unit_tests_day_07_part_b() {
        ...
        let output = find_optimal_thruster_signal(&program_1, compute_feedback_thruster_signal, 5..10);
        assert_eq!(output, expected_output_1);
        ...
        let output = find_optimal_thruster_signal(&program_2, compute_feedback_thruster_signal, 5..10);
        assert_eq!(output, expected_output_2);
    }
}